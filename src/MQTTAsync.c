...
int MQTTAsync_connect(MQTTAsync handle, const MQTTAsync_connectOptions* options)
{
	...
	if (m->c->sslopts)
	{
		...
		{
			if (m->c->sslopts->sign_cert_file)
				free((void*)m->c->sslopts->sign_cert_file);
			if (m->c->sslopts->sign_key_file)
				free((void*)m->c->sslopts->sign_key_file);
			if (m->c->sslopts->enc_cert_file)
				free((void*)m->c->sslopts->enc_cert_file);
			if (m->c->sslopts->enc_key_file)
				free((void*)m->c->sslopts->enc_key_file);
		}
	...
	if (options->struct_version != 0 && options->ssl)
	{
		...
		if (m->c->sslopts->struct_version >= 6)
		{
			m->c->sslopts->enable_ntls = options->ssl->enable_ntls;

			if (options->ssl->sign_cert_file)
				m->c->sslopts->sign_cert_file = MQTTStrdup(options->ssl->sign_cert_file);
			if (options->ssl->sign_key_file)
				m->c->sslopts->sign_key_file = MQTTStrdup(options->ssl->sign_key_file);
			if (options->ssl->enc_cert_file)
				m->c->sslopts->enc_cert_file = MQTTStrdup(options->ssl->enc_cert_file);
			if (options->ssl->enc_key_file)
				m->c->sslopts->enc_key_file = MQTTStrdup(options->ssl->enc_key_file);
		}
	}
#else
	if (options->struct_version != 0 && options->ssl)
